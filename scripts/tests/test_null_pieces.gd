...
## Test: Geometrically complete cells shifting to empty positions become unwalkable
##
## This test verifies the extended behavior: cells that merge with empty positions
## (both inside and outside grid) become unwalkable by receiving null pieces.
func test_complete_cell_to_empty_position_becomes_unwalkable():
	await wait_physics_frames(2)

	var cell_size = grid_manager.cell_size
	var test_pos = Vector2i(8, 8)

	# Create a cell with COMPLETE geometry (full square)
	var local_pos = Vector2(test_pos) * cell_size
	var complete_geometry = PackedVector2Array([
		local_pos,
		local_pos + Vector2(cell_size, 0),
		local_pos + Vector2(cell_size, cell_size),
		local_pos + Vector2(0, cell_size)
	...
	var test_cell = Cell.new(test_pos, local_pos, cell_size)
	test_cell.geometry_pieces.clear()  # Remove default piece
	var complete_piece = CellPiece.new(complete_geometry, CellPiece.CELL_TYPE_EMPTY, -1)
	test_cell.add_piece(complete_piece)

	# Verify cell is currently walkable (no null pieces)
	assert_false(test_cell.has_cell_type(CellPiece.CELL_TYPE_NULL),
		"Cell should not have null pieces before shift")

	# Add to grid
	grid_manager.add_child(test_cell)
	grid_manager.cells[test_pos] = test_cell

	# Call the function to add null pieces (simulating shift to empty position)
	fold_system._add_null_pieces_to_complete_cell(test_cell, test_pos)

	# Verify that null piece was added even though geometry was complete
	assert_true(test_cell.has_cell_type(CellPiece.CELL_TYPE_NULL),
		"Complete cell at empty position should have null piece added")

	# Verify dominant type is now NULL (unwalkable)
	assert_eq(test_cell.get_dominant_type(), CellPiece.CELL_TYPE_NULL,
		"Cell shifting to empty position should become unwalkable (null dominant type)")

	# Verify it has multiple pieces now (complete piece + null piece)
	assert_true(test_cell.geometry_pieces.size() >= 2,
		"Cell should have at least 2 pieces after null addition")


## Test: Cells with existing null pieces are not duplicated
##
## Verifies that we don't add null pieces to cells that already have them
func test_no_duplicate_null_pieces():
	await wait_physics_frames(2)

	var cell_size = grid_manager.cell_size
	var test_pos = Vector2i(7, 7)

	# Create a cell that already has a null piece
	var local_pos = Vector2(test_pos) * cell_size
	var partial_geometry = PackedVector2Array([
		local_pos,
		local_pos + Vector2(cell_size / 2, 0),
		local_pos + Vector2(cell_size / 2, cell_size / 2),
		local_pos + Vector2(0, cell_size / 2)
	])

	var test_cell = Cell.new(test_pos, local_pos, cell_size)
	test_cell.geometry_pieces.clear()
	var partial_piece = CellPiece.new(partial_geometry, CellPiece.CELL_TYPE_EMPTY, -1)
	test_cell.add_piece(partial_piece)

	# Add to grid
	grid_manager.add_child(test_cell)
	grid_manager.cells[test_pos] = test_cell

	# First call - should add null piece for complement
	fold_system._add_null_pieces_to_complete_cell(test_cell, test_pos)
	var pieces_after_first = test_cell.geometry_pieces.size()

	# Second call - should NOT add another null piece
	fold_system._add_null_pieces_to_complete_cell(test_cell, test_pos)
	var pieces_after_second = test_cell.geometry_pieces.size()

	# Should not have duplicate null pieces
	assert_eq(pieces_after_second, pieces_after_first,
		"Should not add duplicate null pieces if cell already has one")
