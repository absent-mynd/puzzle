...
func test_you_can_pin_and_fold_at_depth_two() -> void:
	...
	_pin(Vector2i(-1, 0))
	assert_eq(world.armed_pairs().size(), 1, "...and the second lights a fuse two layers down")
...
func test_the_preview_band_is_drawn_in_every_copy() -> void:
	...
	_pin(Vector2i(0, -1))
	_pin(Vector2i(0, 1))
	assert_eq(world.armed_pairs().size(), 1, "A pair is armed, so there is a strip to preview")
...
func test_the_preview_is_one_band_in_a_world_that_does_not_repeat() -> void:
	...
	_pin(Vector2i(1, 0))
	_pin(Vector2i(-1, 0))
	assert_eq(world.armed_pairs().size(), 1, "A pair is armed")
	world.overlay.set_view(world._build_overlay_view())
