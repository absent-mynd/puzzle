...
## AnchorStock tests — the slot ledger behind the hands you carry.
##
...
func test_you_can_hold_exactly_one_fold_s_worth() -> void:
	assert_eq(AnchorStock.SLOTS, AnchorStock.HANDS_PER_FOLD,
...
func test_empty_slots_start_empty() -> void:
	var slots := AnchorStock.empty_slots()
	assert_eq(slots.size(), AnchorStock.SLOTS, "One entry per slot")
	assert_eq(AnchorStock.held_count(slots), 0, "Nothing in them")
	assert_eq(AnchorStock.free_slots(slots), AnchorStock.SLOTS, "All of them free")
	assert_false(AnchorStock.has_hand(slots), "Nothing to place")


func test_counting_what_is_in_your_slots() -> void:
	assert_eq(AnchorStock.held_count([PLAIN, SWIFT]), 2, "Both slots full")
	assert_eq(AnchorStock.held_count([null, SWIFT]), 1, "One down, one held")
	assert_eq(AnchorStock.free_slots([null, SWIFT]), 1, "...so one slot is free")
	assert_true(AnchorStock.has_hand([null, SWIFT]), "One hand is enough to place with")


func test_first_empty_and_first_held_pick_the_obvious_slot() -> void:
	assert_eq(AnchorStock.first_empty([PLAIN, null]), 1, "The empty one takes an arrival")
	assert_eq(AnchorStock.first_empty([PLAIN, SWIFT]), -1, "No room reports -1, not slot 0")
	assert_eq(AnchorStock.first_held([null, SWIFT]), 1, "The full one is what you place")
	assert_eq(AnchorStock.first_held([null, null]), -1, "Nothing to place reports -1")
...
func test_can_receive_gates_hands_coming_home() -> void:
	...
	assert_true(AnchorStock.can_receive([null, null], 2), "Empty hands can take a whole fold back")
	assert_false(AnchorStock.can_receive([null, SWIFT], 2),
		"Carrying a spare leaves nowhere for the second hand to go")
	assert_true(AnchorStock.can_receive([null, SWIFT], 1), "...but one would fit")
	assert_false(AnchorStock.can_receive([PLAIN, SWIFT], 1), "Full hands can take nothing")
...
func test_held_by_counts_hands_in_one_list() -> void:
	...
	assert_eq(AnchorStock.held_by([_fold([PLAIN, SWIFT])]), 2, "A standing fold holds two")
	assert_eq(AnchorStock.held_by([_fold([PLAIN, PLAIN]), _fold([])]), 2,
...
func test_held_in_counts_across_lists() -> void:
	...
	assert_eq(AnchorStock.held_in([[_fold([PLAIN, PLAIN])], [_fold([SWIFT, SWIFT])], []]), 4,
...
func test_the_three_places_always_sum_to_the_same_total() -> void:
	...
	assert_eq(AnchorStock.total(slots, 0, lists), 2, "Two hands exist, both in your slots")

	slots = [null, SWIFT]                                   # placed one
	assert_eq(AnchorStock.total(slots, 1, lists), 2, "Placing moved one, it did not spend it")

	slots = [null, null]                                    # placed the other
	assert_eq(AnchorStock.total(slots, 2, lists), 2, "Both down, both still yours")

	lists[0].append(_fold([PLAIN, SWIFT]))                  # the pair committed
	assert_eq(AnchorStock.total(slots, 0, lists), 2,
...
func test_picking_one_up_is_the_only_thing_that_raises_the_total() -> void:
	...
	assert_eq(AnchorStock.total([null, null], 0, lists), 2, "Two hands, both committed")
	assert_eq(AnchorStock.total([SWIFT, null], 0, lists), 3,
