...
## The blocking polygons of the view you are in. Kept beside `_frame_pieces` because
## the same question ("which world am I asking about") has to be answered the same way
## by everything: a hand dropped inside a subspace settles on the strip's walls, not on
## the overworld's.
func _frame_solids() -> Array:
	return sub_wall_polys if mode == Mode.SUBSPACE else wall_polys
...
func anchor_point(entry):
	if entry == null or String(entry["region"]) != region_id:
		return null
	return BaseFrame.world_point_from_base(_frame_pieces(), entry["bid"], entry["bp"])
...
func do_sub_fold(a1: Vector2i, a2: Vector2i, pinned: Array[int] = []) -> bool:
	...
	# Balls flying inside the strip ride this interior fold the same way they ride a
	# world one. One rule, applied at every level — like unfold blocking.
	_carry_balls_through(new_pieces, false)
...
func _step_hand_balls(delta: float) -> void:
	...
	for i in range(hand_balls.size() - 1, -1, -1):
		...
		if bool(next["resting"]):
			...
			# Inside a fold, a falling thing WRAPS. The strip is a cylinder — the same
			# rule the player crosses the glue by — so a hand that finds no floor comes
			# back around and goes on falling. When the wrap axis has a vertical
			# component that is a hand in orbit, indefinitely, and it is a real object in
			# a real place rather than a leak: it is still counted, still catchable, and
			# it still lands the moment a fold puts ground in its way.
			ball["pos"] = WorldCore.wrap_into_strip(
				Vector2(ball["pos"]), sub_fold.crease_normal,
				sub_fold.crease_point1, sub_fold.gap_distance())
			# Off the tangential ends there is nothing at all — the strip is unbounded
			# that way and does not repeat, so this is the one direction a thing can
			# genuinely leave by. Turn it back the way the fold turns the player back.
			var tangent := Vector2(-sub_fold.crease_normal.y, sub_fold.crease_normal.x)
			var tproj := Vector2(ball["pos"]).dot(tangent)
...
func _recover_lost_hand(kind: int) -> void:
	var landing := WorldCore.settle_hand(player.global_position, _frame_solids())
	_land_ball({
		"kind": kind,
		"pos": landing,
		"region": region_id,
...
func _land_ball(ball: Dictionary) -> void:
	...
	for step in range(1, 5):
		...
		offsets.append(Vector2(0, 0.5 * CS * step))
	for off in offsets:
		var piece = BaseFrame.piece_containing(_frame_index(), rest + off, CS)
		if piece != null:
	...
	var fallback = BaseFrame.piece_containing(_frame_index(), _spawn, CS)
	if fallback != null:
		_ensure_pickups(String(ball["region"])).append(
			HandPickup.dropped_at(
...
func _carry_balls_through(new_pieces: Array, into_sub: bool) -> void:
	...
	for ball in hand_balls:
		if bool(ball["in_sub"]) != here or String(ball["region"]) != region_id:
			continue
		var from = BaseFrame.piece_containing(_frame_index(), Vector2(ball["pos"]), CS)
		var dest = null
		if from != null:
			dest = BaseFrame.world_point_from_base(
